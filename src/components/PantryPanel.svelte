...
{#if pantry.items.length === 0}
	<p class="empty">
		List what you usually buy. When something runs out, switch it off. It stays here for next time.
	</p>
{/if}
{#if pantry.inStock.length > 0}
	<h3>In stock · {pantry.inStock.length}</h3>
	<ul>
		{#each pantry.inStock as item (item.id)}<IngredientRow {item} />{/each}
	</ul>
{/if}
{#if pantry.out.length > 0}
	<h3>Out · {pantry.out.length}</h3>
	<ul>
